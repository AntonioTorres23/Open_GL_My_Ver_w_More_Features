...
vec3 PosLightCalculations (PosLight pos_light_struct_arg, vec3 normal_coordiantes_arg, vec3 position_of_models_fragments_arg, vec3 pos_of_camera_arg)
{
	...
	// create a halfway direction that is used to help with calculating specular lighting
	// we grab the angle between the halfway direction and normal direction
	vec3 halfway_direction = normalize(light_direction + direction_of_camera_from_model);

	// instead of using a reflect and review direction, we are grabbing the angle/scalar value between the halfway direction and normal coordinates
	// this is a lighting model called Blinn-Phong and only affects the specular lighting calculations of the original Phong lighting model
	float specular_lighting_calc = pow(max(dot(normal_coordiantes_arg, halfway_direction), 0.0), 32);
