...
vec3 DirectionalLightingCalculations (DirectionalLighting direction_lighting_arg, vec3 normal_coordinates_arg, vec3 pos_of_camera_arg)
{
	...
	vec3 ambient_result = direction_lighting_arg.ambient_color; 

	vec3 diffuse_result = direction_lighting_arg.diffuse_color * diffuse_lighting;
...
vec3 PosLightCalculations (PosLight pos_light_struct_arg, vec3 normal_coordiantes_arg, vec3 position_of_models_fragments_arg, vec3 pos_of_camera_arg)
{
	...
	// calculations for diffuse_lighting
	vec3 diffuse_lighting_calc = vec3(max(dot(light_direction, normal_coordiantes_arg), 0.0));
...
void main()
{
	...
	lighting = DirectionalLightingCalculations(direction_lighting_var, normalized_normal_coordniates, direction_of_camera_from_model);
